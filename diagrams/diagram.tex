...
%P: second argument    
% %O:  Arg attacking A - 1		
\node[below right = 0.4cm and 2.75cm of ps] (pxc) {$\bar{c}$}
child {node (pxcu) {$u$}
...
%O: third argument    
% %O:  Arg attacking A - 1		
\node[right = 4.6cm of ps] (pxa) {$\bar{a}$}
child {node[xshift=0.25cm]  (pxav) {$v$}}
...
\begin{pgfonlayer}{background}
    ...
    \node [scale=0.8] at ([shift={(0pt,5pt)}]ps.north -| pxad2.east) {5};
\end{pgfonlayer} 
...
\begin{pgfonlayer}{background}
    ...
    \node [scale=0.8] at ([shift={(12pt,5pt)}]ps.north -| pxcd.east) {4};
\end{pgfonlayer} 
